...
with gr.Blocks() as app:
    ...
    with gr.Row():
        with gr.Column():
            text_input = gr.Textbox(lines=2, label="文本",value="事实上，我就是钢铁侠")
            speed_slider = gr.Slider(label='语速', minimum=0.1, maximum=1.99, value=0.8, step=0.01)
            language_dropdown = gr.Dropdown(list(languages.keys()), label="语言", value="Chinese")
            ...
            with gr.Row():
                ...
                with gr.Column():
                    filename_input = gr.Textbox(label="Add new Speaker", placeholder="输入角色名称")
                    save_button = gr.Button("保存角色")
                
            refresh_button.click(fn=update_dropdown, inputs=[], outputs=speaker_dropdown)

            with gr.Row():
                record_button = gr.Audio(label="开启录音")
